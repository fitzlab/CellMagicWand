...
% ========================================== %
% Correct for the intensity differences over each difference image.
% Fourier high-pass will do the trick here.
...
for i=1:height
    for j=1:width
        ...
        intensityCorrectionMask(i,j) = (1+4*distToCenter/width)*intensityCorrectionMask(i,j);
    end
end

writeDoubleTif(intensityCorrectionMask,'intensityCorrection.tif');

derivTifs = uint16(bsxfun(@times,double(derivTifs),intensityCorrectionMask));
derivMaxCorrected = max(derivTifs,[],3);
writeDoubleTif(derivMaxCorrected,'derivMaxIntensityCorrection.tif');

% ========================================== %
% Find the image gradient (Prewitt edge detector)
disp('Calculating gradient images');
